...
import {
  AcademicYearSelect, Button, Field, FileDrop, PageBack, YearSelect
} from '../components/Ui';
...
const STEPS = ['Informasi Dasar', 'Metadata', 'Jenis Penelitian', 'Dokumen', 'Luaran', 'Tinjau dan Kirim'];
...
export default function ExternalResearchWizardPage({ archiveMode, match }) {
  ...
  const saveDraft = () => {
    setData(current => {
      ...
      return {
        ...current,
        externalResearchReports: (current.externalResearchReports || []).map(item => (item.id === report.id ? saved : item)),
  ...
  if (!reportId) return <div className="ris-page"><h1>Membuat draf...</h1></div>;
    ...
    <div className="ris-page ris-page-narrow">
      ...
      <div className="ris-page-heading">
        <div>
          ...
          <p>{archiveMode ? 'Perbarui informasi, metadata, dokumen, dan luaran penelitian eksternal.' : 'Panduan pelaporan: Informasi Dasar, Metadata, Jenis Penelitian, Dokumen, Luaran, serta Tinjau dan Kirim.'}</p>
        </div>
      ...
      {errorList.length > 0 && (
        <div className="ris-alert ris-alert-error">
          ...
          {errorList.length > 8 && <span>Dan {errorList.length - 8} kesalahan lainnya.</span>}
        </div>
      )}
      ...
      {step === 0 && (
        <section className="ris-form-section">
          ...
          <Field label="Nominal Pendanaan" required><input type="number" min="0" step="1000" inputMode="numeric" value={inputValue(report.fundingAmount)} onChange={event => update({ fundingAmount: event.target.value })} /></Field>
      ...
      {step === 1 && (
        <section className="ris-form-section">
          <h2>Langkah 2 — Metadata Penelitian</h2>
          ...
          {report.metadata.integrationToTeaching && (
            <>
              ...
              <Field label="Bukti Integrasi" required alignStart><FileDrop file={report.metadata.integrationProofFile} accept=".pdf,.docx,.pptx" onFile={file => updateMeta({ integrationProofFile: { ...fileMeta(file), fileUrl: file.name } })} label="Unggah bukti integrasi pembelajaran" /></Field>
            </>
          )}
        </section>
      )}
      ...
      {step === 2 && (
        <section className="ris-form-section">
          <h2>Langkah 3 — Jenis Penelitian</h2>
          ...
          {report.category === 'grant' && (
            <>
              ...
              <Field label="Nominal Pendanaan"><input type="number" min="0" step="1000" inputMode="numeric" value={inputValue(report.typeDetail.fundingAmount)} onChange={event => updateDetail({ fundingAmount: event.target.value })} /></Field>
            </>
          )}
      ...
      {step === 3 && (
        <section className="ris-form-section">
          <h2>Langkah 4 — Pengelolaan Dokumen</h2>
          <div className="ris-alert ris-alert-success"><strong>Ketentuan Dokumen Wajib</strong><span>Status yang dipilih membutuhkan dokumen: {requiredDocuments.map(getDocumentTypeLabel).join(', ')}.</span></div>
          {EXTERNAL_DOCUMENT_TYPES.map(item => {
            ...
            return (
              <Field key={item.value} label={item.label} required={requiredDocuments.includes(item.value)} alignStart>
                <FileDrop file={file} accept=".pdf,.docx,.xlsx,.pptx" onFile={selected => addDocument(item.value, selected)} label={`Unggah ${item.label}`} />
                {file && <button type="button" className="ris-text-danger" onClick={() => removeDocument(file.id)}>Hapus berkas ini</button>}
              </Field>
            );
          })}
        </section>
      ...
      {step === 4 && (
        <section className="ris-form-section">
          <div className="ris-section-title"><h2>Langkah 5 — Pengelolaan Luaran</h2><Button tone="blue" onClick={addOutput}>Tambah Luaran</Button></div>
          {(report.outputs || []).map((output, index) => (
            <div key={output.id} className="ris-output-card">
              ...
              <Field label="Berkas Luaran" alignStart><FileDrop file={output.file} accept=".pdf,.docx,.xlsx,.pptx" onFile={file => attachOutputFile(output.id, file)} label="Unggah berkas luaran opsional" /></Field>
            </div>
          ))}
      ...
      {step === 5 && (
        <section className="ris-form-section">
          <h2>Langkah 6 — Tinjau dan Kirim</h2>
          ...
          {!validation.valid && <div className="ris-alert ris-alert-error"><strong>Belum siap dikirim</strong><span>Masih ada {validation.errors.length} validasi yang belum terpenuhi.</span></div>}
        </section>
      )}
      ...
      <div className="ris-bottom-bar">
        ...
        <div>
          ...
          {step === STEPS.length - 1 && (archiveMode ? <Button tone="blue" onClick={saveDraft}>Simpan Perubahan</Button> : <Button tone="blue" onClick={submitReport}>Kirim Laporan</Button>)}
        </div>
      </div>
    </div>
  );
