...
export default function ResearcherProfileDashboardPage() {
  ...
  const [form, setForm] = useState({
    fullName: '', institutionEmail: '', nidn: '', faculty: '', studyProgram: '', position: 'Dosen Fulltime', role: ROLE.RESEARCHER
  });
  ...
  if (!canOpenProfileModule(user)) {
    return <div className="ris-page"><h1>Profil Peneliti</h1><p className="ris-muted">Role akun ini tidak memiliki akses ke area profil peneliti.</p></div>;
  }
  ...
  const createUser = () => {
    ...
    const newUserId = `user-researcher-${profileNumber}`;
    const newProfileId = `lecturer-${profileNumber + 10}`;
    const account = {
      id: newUserId,
      name: form.fullName.trim(),
      email: cleanedEmail,
      password: 'password123',
      role: form.role || ROLE.RESEARCHER,
    ...
    setData(current => ({
      ...
      researcherProfiles: [...(current.researcherProfiles || []), saved],
      applicantProfiles: [...(current.applicantProfiles || []), {
        id: saved.profileId,
        userId: saved.userId,
        name: saved.fullName,
      ...
      systemActivityLogs: [...(current.systemActivityLogs || []), createActivityLog(user, 'create_user', 'researcher_profile', saved.profileId, null, { account, profile: saved, temporaryPassword: 'password123' }, uid)],
      notifications: [...(current.notifications || []), createNotification(saved.userId, user.id, 'new_profile_created', 'Akun peneliti dibuat oleh Admin LPPM. Password demo: password123', uid)],
      emailOutbox: [...(current.emailOutbox || []), createEmailNotification({
        to: account.email,
        subject: 'Akun RIS Anda telah dibuat',
        message: `Akun RIS untuk ${account.name} telah dibuat oleh ${user.name}. Email login: ${account.email}. Password demo: password123.`,
        userId: saved.userId,
    ...
    <div className="ris-page ris-workspace-page ris-profile-page">
      <div className="ris-page-head split">
        <div>
          ...
          <p className="ris-muted">Single Source of Truth untuk identitas, dokumen, bidang minat, dan verifikasi peneliti RIS.</p>
        </div>
        {admin ? (
          <div className="ris-button-row">
            ...
            <Button tone="green" onClick={() => setShowCreate(true)}>Buat Akun Dosen</Button>
          </div>
        ) : <Button tone="blue" onClick={ensureOwnProfile}>{ownProfile ? 'Edit Profil Saya' : 'Initial Profile Setup'}</Button>}
      </div>
        ...
        <section className="ris-card ris-profile-overview">
          {ownProfile ? (
            <>
              <div>
                <h2>Profil Saya</h2>
                <p className="ris-muted">{ownProfile.fullName || user.name} • {ownProfile.position || 'Role administratif'} • {ownProfile.institutionEmail}</p>
              ...
              <div className="ris-progress-card">
                ...
                <div className="ris-button-row"><Button tone="blue" onClick={openOwnProfile}>Detail</Button><Button tone="green" onClick={ensureOwnProfile}>Edit</Button></div>
              </div>
            </>
          ) : (
            <div>
          ...
          {ownProfile ? (
            <>
              ...
              <div className="ris-progress-card">
                ...
                <div className="ris-button-row"><Button tone="blue" onClick={() => history.push(`/ris/profil-peneliti/${ownProfile.profileId}/detail`)}>Detail</Button><Button tone="green" onClick={() => history.push(`/ris/profil-peneliti/${ownProfile.profileId}/edit`)}>Edit</Button></div>
              </div>
            </>
          ) : (
            <div>
              ...
              <Button tone="green" onClick={ensureOwnProfile}>Mulai Initial Profile Setup</Button>
            </div>
          )}
        </section>
      )}
      ...
      {admin && (
        <section className="ris-section-spaced">
          <div className="ris-section-title-row"><h2>Daftar Profil Peneliti</h2><Button tone="gray" onClick={exportCsv}>Export CSV</Button></div>
          <div className="ris-filter-grid">
            <Field label="Search"><input value={filters.search} onChange={event => setFilters({ ...filters, search: event.target.value })} placeholder="Nama, email, NIDN" /></Field>
            ...
            <Field label="Expertise"><select value={filters.expertise} onChange={event => setFilters({ ...filters, expertise: event.target.value })}><option value="">Semua</option>{(data.researcherExpertise || []).map(item => <option key={item.expertiseId} value={item.name}>{item.name}</option>)}</select></Field>
          </div>
          <div className="ris-table-wrap">
            <table className="ris-table ris-action-table ris-profile-table">
              <thead><tr><th>No.</th><th>Nama</th><th>Email</th><th>NIDN</th><th>Fakultas / Prodi</th><th>Kelengkapan</th><th>Verifikasi</th><th>Status</th><th>Aksi</th></tr></thead>
              <tbody>
                {filteredProfiles.map((profile, index) => {
                  ...
                  return (
                    <tr key={profile.profileId}>
                      ...
                      <td><button type="button" className="ris-action blue" onClick={() => history.push(`/ris/profil-peneliti/${profile.profileId}/detail`)}>Detail</button>{canEditProfile(profile, user, (data.systemUsers || []).find(account => account.id === profile.userId)) && <button type="button" className="ris-action green" onClick={() => history.push(`/ris/profil-peneliti/${profile.profileId}/edit`)}>Edit</button>}</td>
                    </tr>
                  );
                })}
                {filteredProfiles.length === 0 && <EmptyRow colSpan={9}>Tidak ada profil sesuai filter.</EmptyRow>}
              </tbody>
            </table>
          </div>
        </section>
      ...
      {showCreate && (
        <Modal title="Buat Akun Dosen Internal" onClose={() => { setCreateError(''); setShowCreate(false); }} width={760}>
          <div className="ris-modal-body">
            ...
            <div className="ris-form-grid two">
              ...
              <Field label="NIDN" required><input value={form.nidn} onChange={event => setForm({ ...form, nidn: event.target.value })} /></Field>
            ...
            <p className="ris-muted">Password demo otomatis: <strong>password123</strong>. Pada backend production, field ini diganti activation token/email.</p>
